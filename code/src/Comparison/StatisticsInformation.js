  ...
  function radarDatasets() {
    ...
    if (dataOne && dataTwo) {
      statOne = attributes
        ...
        .map((attr) => dataOne[attr]["avg"]);

      statTwo = attributes
        .filter((attr) => (dataType === "perPoss" ? attr !== "POSS" : attr))
        .map((attr) => dataTwo[attr]["avg"]);
    }
      ...
      tooltips: {
        ...
        callbacks: {
          ...
          label: function (tooltipItem, data) {
            ...
            if (arrayPosition === 0) {
              return `${data["datasets"][arrayPosition]["label"]}: ${data[
                "datasets"
              ][arrayPosition].labels[tooltipItem["index"]].toFixed(1)} (${
  ...
  function normalizeRadarData(
    ...
    if (Object.keys(maxDataOne).includes(attrLowerCased)) {
      ...
      const maxYearlyOverall =
        maxDataOverall[attrLowerCased].value -
        minDataOverall[attrLowerCased].value;
    ...
    attributes
      ...
      .map((attr, index) => {
        if (dataOne && dataTwo) {
          ...
          const stdValueOne =
            dataType === "perPoss"
              ? +dataOne[attr].std / dataOne["POSS"].avg
              : +dataOne[attr].std;
          const stdValueTwo =
            dataType === "perPoss"
              ? +dataTwo[attr].std / dataTwo["POSS"].avg
              : +dataTwo[attr].std;
        ...
        <StyledPlayerInfoAndSwitches>
          <StyledPlayerInfo location={"left"}>
            <CompareSideInfo
              isTeam={isTeam ? "true" : "false"}
              location={"left"}
              name={leftSelectedName}
              year={leftSelectedYear}
            />
          </StyledPlayerInfo>
          ...
          <StyledPlayerInfo location={"right"}>
            <CompareSideInfo
              isTeam={isTeam ? "true" : "false"}
              location={"right"}
              name={rightSelectedName}
              year={rigthSelectedYear}
            />
          </StyledPlayerInfo>
        </StyledPlayerInfoAndSwitches>
      )}
        ...
        <StyledComparisonBars>
          <div className="bar-group">
            {sortComparisonBars()["sortedPValueListOne"].length > 0 && (
              <p className="bar-heading">
                <strong>{leftSelectedName.replace(/_/g, " ")} </strong>is better
                than <strong>{rightSelectedName.replace(/_/g, " ")} </strong>
                with
                <strong> 80% or greater probability</strong>
              </p>
            )}

            {sortComparisonBars()["sortedPValueListOne"].map((list, index) => (
              <ComparisonBars
                key={index}
          ...
          <div className="bar-group">
            {sortComparisonBars()["sortedPValueListTwo"].length > 0 && (
              <p className="bar-heading">
                <strong>{rightSelectedName.replace(/_/g, " ")} </strong>is
                better than{" "}
                <strong>{leftSelectedName.replace(/_/g, " ")} </strong>with
                <strong> 80% or greater probability</strong>
              </p>
            )}
            {sortComparisonBars()["sortedPValueListTwo"].map((list, index) => (
              <ComparisonBars
                key={index}
                getTeamColour={getTeamColour}
                listGroup="two"
