...
import {
  logoutAction,
  checkUserRecordCollectionExists,
...
const Betting = (props) => {
  ...
  const [exceedingTimeLimitPopup, setExceedingTimeLimitPopup] = useState({
    isVisible: false,
  });
  ...
  const [
    numberOfGamesExceedingTimeLimit,
    setnumberOfGamesExceedingTimeLimit,
  ] = useState(0);
  ...
  useEffect(() => {
    if (
      props.futureGamesInfo.games.length > 0 &&
      !props.futureGamesInfo.isLoading
    ) {
      ...
      props.futureGamesInfo.length < 1 &&
      !props.futureGamesInfo.isLoading
    ) {
      setError({
  ...
  useEffect(() => {
    if (props.userBetsDetails.bets[0] && !props.userBetsDetails.loading) {
      let computedArray = compareUserBetsAndGameInfo(
        props.userBetsDetails.bets,
        props.futureGamesInfo.games
      );
      setPointsSpinner(false);
  ...
  const onPointBoxClick = (
    ...
    if (!betSubmitted && oddsValue) {
      ...
      //game hasn't started yet then the function runs in with the conditional statement below.
        ...
        const returnedObj = pointBoxClickHandler(
          e,
          params,
          index,
          gameId,
        ...
        Object.keys(returnedObj.targetObj).length === 0
  ...
  const onRemovePoints = (e, params, gameId) => {
    ...
    if (
      !targetObj[gameId]["moneyLine"].odds &&
      !targetObj[gameId]["handicap"].odds &&
      !targetObj[gameId].overAndUnder.odds
    ) {
  ...
  const submitConfirmation = async () => {
    ...
    if (result === true) {
      let response = await props.submitBetPoints(
        selectedValues,
        gameInfo,
      ...
      if (response.isError) {
        ...
        let getUserBetsResponse = await props.getUserBets(
          props.userDetails.user.uid
        );
        if (getUserBetsResponse.isError) {
          setError({
            status: getUserBetsResponse.status,
            message: getUserBetsResponse.message,
            isError: true,
          });
        } else {
          ...
          {loginModalVisible ? (
            <LoginModalContainer>
              <div>
                ...
                >
                  <LoginModal onLoginClick={onLoginClick} />
                </OutsideClickHandler>
              </div>
            </LoginModalContainer>
          ) : null}
              ...
              <BettingSectionColumn>
                <ContentHeader>
                  ...
                  <p className="information">
                    Get your own NBA intelligence rating by making the game
                    outcome predictions. The concept is the same as sports
                    betting except that you win/lose points instead (meaning no
                  ...
                  {error.message === "No games today" ? (
                    ...
                    <BettingPointsAndTeamsContainer>
                      {gameInfo.map((element, index) => {
                        let teamIconsObj = setTeamIcons(
                          element.gameDetails.homeTeam,
                          element.gameDetails.awayTeam
                        );
                        let currentDate = momentTimezone(new Date())
                          .tz("America/New_York")
                          .format("YYYY-MM-DD hh:mm A");
                        let gameStartDate =
                          element.gameDetails.gameDate +
                          " " +
                          element.gameDetails.gameStartTime;
                        let isGameStartTimeBeforeTheCurrentTime = moment(
                          gameStartDate
                        ).isAfter(moment(currentDate));

                        return (
                          <BettingPointsAndTeamsWrapper key={index}>
                            <div>
                              <BettingSectionheader
                                gameStartTime={element.gameDetails.gameStartTime.split(
                                  " PM"
                                )}
                              />
                            </div>
                ...
                <div className="instruction">
                  ...
                  <p>
                    ...
                    incorrectly, you lose 1 point. Say you bet on “LAC” for 1.95
                    betting odds. If the Clippers win, then you will earn 0.95
                    points. If the Clippers lose then you lose -1. This is
                    equivalent to having a stake size of $1.
                  </p>
                  <p style={{ paddingTop: "30px" }}>Notes:</p>
                  <ul style={{ paddingLeft: "30px" }}>
