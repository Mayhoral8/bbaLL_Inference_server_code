...
const StatsBarPlot = ({ info, mirror, y, away }) => {
  ...
  const quarterlyInfo = Object.keys(info).filter(el => el.match(/Q/g)).sort();
  ...
  const quarterlyXRatio = (quarter, team) => {
    ...
    y.forEach(val => {
      if (/percentage|rate/g.test(val)) {
        const xValue = (info[quarter][team][val] / getAttrSum(val)) * (info[team][val] / (info.Home[val] + info.Away[val]));
        x.push(xValue);
      } else {
  ...
  const createBarGraph = (x, y, name, orientation, color, width, type) => {
    return {
      x, y, name, orientation, marker: { color, width }, type
    }
  }
  ...
  quarterlyInfo.forEach(info => {
    ...
    const trace = createBarGraph(quarterlyXRatio(info, away ? 'Away' : 'Home'), y, info, 'h', rgba('#534a91', opacity), 1, 'bar');
    data.push(trace);
  });
