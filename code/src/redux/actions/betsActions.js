...
import {
  GET_FUTURE_GAMES_INFO,
  GETUSERBETS,
  SET_STRUCTURED_GAME_INFO,
} from "./types";
    ...
    for (let i = 0; i < gameInfo.length; i++) {
      for (let j = 0; j < gameIdKeys.length; j++) {
        if (gameIdKeys[j] === gameInfo[i].gameId) {
          let gameDate = selectedValues[`${gameIdKeys[j]}`].gameDetails.gameDate
            ? selectedValues[`${gameIdKeys[j]}`].gameDetails.gameDate
            : "";
          if (
            gameInfo[i].overUnder.selected ||
            gameInfo[i].moneyLine.selected ||
            gameInfo[i].handicap.selected
          ) {
            let targetObj = {};
            if (
              (gameInfo[i].overUnder.selected &&
              ...
              if (
                ...
                targetObj.overAndUnder =
                  selectedValues[gameIdKeys[j]].overAndUnder;
              }
            } else {
            ...
            try {
              let userList = await fbFirestoreSpigameBet
              ...
              let betHistory = await fbFirestoreSpigameBet
                .collection("userBettingHistoryTracker")
                .doc(userId)
                .collection("year")
                .doc(year)
                .collection("month")
              ...
              if (!betHistory.data()) {
                ...
                await fbFirestoreSpigameBet
                  ...
                  .collection("month")
                  .doc(month)
                  .set(betHistoryTargetObj, { merge: true });
              } else {
