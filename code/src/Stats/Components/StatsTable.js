...
const StatsTable = ({ data, tableHeadings, tableHeadingsAbbr, includeYear, leftColHeading }) => {
    ...
    return (
      <div className='table-row' key={i}>
        {tableHeadings.map(attr => {
          return (
            <div key={attr} className={`${getClassNameFor(attr, sortConfig)} table-data`}>
              {detail[attr] && attr === 'salary'
                ? detail[attr][0]
                : detail[attr] ? detail[attr].avg.toFixed(2) : ''}
            </div>
          )
        })}
      </div>
